module FSSpecTRRMPhysicsPackage_class
  ...
  !!   prevPrevFlux -> Array of scalar flux values from iteration n-2 of length = nG * nCells
  ...
  !!
  !!   SR           -> Spectral radius where SR = ||phi^(n) - phi^(n-1)|| / ||phi^(n-1) - phi^(n-2)||
  !!
  !! Interface:
  ...
  subroutine normaliseFluxAndVolume(self, lengthPerIt, it)
    ...
    do cIdx = 1, self % nCells
      ...
      if (self % itVol) then
        ...
        self % volume(cIdx) = self % volume(cIdx) * 1.0
      end if
