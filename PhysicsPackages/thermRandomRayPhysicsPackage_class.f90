module thermRandomRayPhysicsPackage_class
  ...
  !!
  type, public, extends(physicsPackage) :: thermRandomRayPhysicsPackage
    private
    ! Components
  ...
  end type thermRandomRayPhysicsPackage
...
contains
  ...
  subroutine init(self, dict, loud)
    class(thermRandomRayPhysicsPackage), intent(inout) :: self
    ...
    character(100), parameter :: Here = 'init (thermRandomRayPhysicsPackage_class.f90)'
    ...
    ! Get dictionary to initialise temperature field
    tempDict => dict % getDictPtr('tempData')

    ! Initialise RR arrays and nuclear data
    call self % arrays % init(self % mgData, self % geom, &
            self % pop * (self % termination - self % dead), self % rho, lin, ani, .false., self % loud, tempDict)
  ...
  subroutine cycles(self)
    class(thermRandomRayPhysicsPackage), target, intent(inout) :: self
    type(ray), save                               :: r
  ...
  subroutine printSettings(self)
    class(thermRandomRayPhysicsPackage), intent(in) :: self
...
end module thermRandomRayPhysicsPackage_class
