module SRrandomRayPhysicsPackage_class
  ...
  contains
    ...
    procedure, private :: calculateFourierTransform
...
contains
  ...
  subroutine calculateSpectralRadius(self,it)
    ...
    avgFlux = sum(self % scalarFlux)/(self % nCells-1)
    ...
    delta1 = self % scalarFlux - predicted
    delta2 = self % prevFlux - predicted
    !delta1 = self % scalarFlux - avgFlux
    !delta2 = self % prevFlux - avgFlux
  ...
  !! Calculates the Fourier transform of the residual
  subroutine calculateFourierTransform(self)
    class(SRrandomRayPhysicsPackage), intent(inout) :: self
    real(defFlt), dimension(self % nCells)          :: residual
    class(defFlt)                                   :: predicted

    residual = self % scalarFlux - predicted


  end subroutine calculateFourierTransform
