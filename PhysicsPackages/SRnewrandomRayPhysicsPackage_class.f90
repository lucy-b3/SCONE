module SRrandomRayPhysicsPackage_class
  ...
  !!   PP {
  !!     type SRrandomRayPhysicsPackage;
  ...
  !!
  type, public, extends(physicsPackage) :: SRrandomRayPhysicsPackage
    private
    ! Components
  ...
  end type SRrandomRayPhysicsPackage
...
contains
  ...
  subroutine init(self,dict)
    ...
    character(100), parameter :: Here = 'init (SRrandomRayPhysicsPackage_class.f90)'
  ...
  subroutine cycles(self)
    ...
    real(defFlt)                                  :: ONE_KEFF
    real(defReal)                                 :: hitRate, elapsed_T, end_T, T_toEnd, transport_T
    ...
    do while( keepRunning )
      ...
      !print *, self % cellHit
      if (hitRate < 0 .or. hitRate > 1) then
        call fatalError('f','f')
        print *,'wtf'
        print *, self % cellHit
        print *, sum(self % cellHit)
      end if
      self % cellHit = 0
  ...
  subroutine initialiseRay(self, r)
    ...
    character(100), parameter :: Here = 'initialiseRay (SRrandomRayPhysicsPackage_class.f90)'
  ...
  subroutine calculateSpectralRadius(self,it)
    ...
    integer(shortInt), intent(in)                  :: it
    integer(shortInt)                              :: cIdx
    real(defFlt), dimension(self % nCells-1)        :: delta1, delta2
    real(defFlt)                                   :: norm1, normm2, avgFlux, predicted

    avgFlux = sum(self % scalarFlux(2:))/(self % nCells-1)
    !predicted = self % nuSigmaF(1)/(self % sigmaT(1) - self % sigmaS(1))
    predicted = 1.0_defFlt / (self % nCells - 1)
    !delta1 = self % scalarFlux(2:) - self % prevFlux(2:)
    !delta2 = self % prevFlux(2:) - self % prevPrevFlux(2:)
    delta1 = self % scalarFlux(2:) - predicted
    !delta2 = self % prevFlux(2:) - predicted
    !delta1 = self % scalarFlux(2:) - avgFlux
    !delta2 = self % prevFlux(2:) - avgFlux

    !norm1 = sqrt(sum(delta1**2))
    !normm2 = sqrt(sum(delta2**2))

    !self % SR(it) = norm1/normm2
    self % SR(it) = norm2(delta1)/norm2(delta2)

    !if (it .eq. 50) then
      !do cIdx = 1, self % nCells-1
        !self % iterateError(cIdx,1) = delta2(cIdx)    
        !self % iterateError(cIdx,2) = delta1(cIdx)
      !end do
    !end if
    if (it < 151) then
      do cIdx = 1, self % nCells - 1
  ...
  subroutine printResults(self)
    ...
    integer(shortInt), save                       :: idx, matIdx, i, j, g
  ...
  subroutine printSettings(self)
    class(SRrandomRayPhysicsPackage), intent(in) :: self
...
end module SRrandomRayPhysicsPackage_class
